...
trans_loan93 <- trans_loan93 %>%
  select(account_id, district_id, balance, status, emprest_date) %>% # selecionando apenas as variáveis de interesse
  group_by(account_id, district_id, status, emprest_date) %>% # agrupando o cálculo por conta e status
